...
- `filter_type: "none" | "element_pair" | "index"`（默认 `"none"`）
- `filter_value`：
...
功能：获取指定键 COHP 曲线，用于前端 ECharts 渲染（行式 JSON）。

请求字段：

- `bond_labels: [str]`（必填，例如 `["1"]`）
- `erange: [emin, emax]`（可选）
- `include_orbitals: bool`（默认 `false`，画图通常不需要轨道以节省带宽）
...
响应示例（节选，返回 `orient="records"` 格式）：

```json
{
  "success": true,
  "code": 200,
  "message": "COHP curves loaded",
  "data": {
    "ispin": 2,
    "bond_labels": ["1"],
    "include_orbitals": false,
...
功能：导出单个 bond 的数据（包含分轨道），专为前端生成 CSV 优化。
...
响应示例（返回 `orient="list"` 的列式 JSON 字典，前端可直接遍历 Keys 拼接 CSV）：

```json
{
  "success": true,
  "code": 200,
  "message": "Extracted orbital data for bond 1",
  "data": {
    "bond_label": "1",
    "include_orbitals": true,
    "data": {
      "energy_eV": [-8.0, -7.9, -7.8],
      "1_up": [-0.10, -0.12, 0.02],
...
6. `POST /api/vasp/cohp/curves`：按选中键拉曲线，用于渲染图表  
7. `POST /api/vasp/cohp/export`：请求带轨道的列式 JSON 数据，前端拼接 `\uFEFF` BOM 头后生成 CSV 下载
...
| COHP 键编号筛选 | ✅ | `filter_type=index` |
| COHP 曲线接口 | ✅ | `/cohp/curves` |
| COHP 单键分轨道导出 | ✅ | `/cohp/export`，前端组装 CSV |
