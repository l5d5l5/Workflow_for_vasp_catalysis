    ...
    def ads_from_prev_calc(
        ...
        **extra_kwargs,
        ...
        return cls(
            ...
            auto_dipole=auto_dipole,
            user_incar_settings=cls._build_incar(functional, base_incar, 
                                                 user_incar_settings=user_incar_settings),
            user_kpoints_settings=kpoints,
            **extra_kwargs,
        )
...
class MPStaticSetEcat(VaspInputSetEcat, MPStaticSet):
    ...
    def from_prev_calc_ecat(
        ...
        **extra_kwargs,
        ...
        prev_dir = Path(prev_dir).resolve()
        loaded_structure = Structure.from_file(prev_dir / "CONTCAR")
        base_incar, functional = cls._read_and_convert_incar(prev_dir / "INCAR", loaded_structure)

        extra_incar = {"NEDOS": int(number_of_docs)} if number_of_docs is not None else None
        incar = cls._build_incar(
            functional,
            DEFAULT_INCAR_STATIC,
            extra_incar=base_incar,
            user_incar_settings=user_incar_settings,
        )
        if extra_incar:
            incar.update(extra_incar)
        ...
        return cls(
            ...
            user_incar_settings=incar,
            user_kpoints_settings=cls._make_kpoints_from_density(loaded_structure, kpoints_density),
            **extra_kwargs,
        )
...
class LobsterSetEcat(VaspInputSetEcat, LobsterSet):
    ...
    def from_prev_calc_ecat(
        ...
        **extra_kwargs,
        ...
        incar = cls._build_incar(functional, base_incar, user_incar_settings=user_incar_settings)
        incar.update(DEFAULT_INCAR_LOBSTER)
...
class FreqSetEcat(MPStaticSetEcat):
    ...
    def from_prev_calc_ecat(
        ...
        **extra_kwargs,
        ...
        extra_incar = {
            "IBRION": 5, "POTIM": 0.015, "NFREE": 2, "NSW": 1,
            "EDIFF": 1e-7, "LCHARG": False, "LWAVE": False, "LREAL": False,
        }

        incar = cls._build_incar(
            functional, base_incar, extra_incar=extra_incar, user_incar_settings=user_incar_settings
        )
